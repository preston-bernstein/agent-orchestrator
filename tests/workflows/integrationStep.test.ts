...
const tmpRoot = path.join(process.cwd(), "runs", "_test_integration_step");
...
const REACT_ONLY_PLAN: PlannerOutputT = {
  status: "ready",
  rationale: "react alone",
  tasks: [
    {
      id: "react-T1",
      spec_slug: "x",
...
const greenBranch: SupervisorBranchResult = {
  supervisors: [],
  ...
  contract_producers: [
    {
      supervisorId: "spring",
      taskId: "spring-T1",
      contractArtifact: "target/openapi.json",
    },
  ],
};
...
describe("runIntegrationStep — Phase 6 cross-repo", () => {
  it("runs integration agent + audits integration_run when producer+consumer green", async () => {
    const { ctx } = makeRun({ runId: "integ-run" });
    const out = await runIntegrationStep(
      {
        ctx,
        plan: CROSS_REPO_PLAN,
        branchResult: greenBranch,
        cwds: { spring: "/fake/spring", react: "/fake/react" },
  ...
  it("flags breaking when contract hash differs vs prior green hash", async () => {
    const { ctx } = makeRun({ runId: "integ-breaking" });
    const out = await runIntegrationStep(
      {
        ctx,
        plan: CROSS_REPO_PLAN,
        branchResult: greenBranch,
        cwds: { spring: "/fake/spring", react: "/fake/react" },
        priorContractHash: hashContract('{"openapi":"3.0"}', ".json"),
      },
      {
        readContract: async () =>
          '{"openapi":"3.0","paths":{"/new":{}}}',
      },
    );
    if (!out.ran) throw new Error("expected ran:true");
  ...
  it("skips w/ aggregate_not_green when supervisorBranch returned non-green", async () => {
    const { ctx } = makeRun({ runId: "integ-skip-red" });
    const out = await runIntegrationStep({
      ctx,
      plan: CROSS_REPO_PLAN,
      branchResult: { ...greenBranch, aggregateStatus: "red" },
      cwds: { spring: "/fake/spring" },
    });
    ...
    expect(audit).toMatch(
      /"step":"integration_skipped".+reason=aggregate_not_green/,
    );
  });

  it("skips w/ no_consumer when plan has no consumes_contract task", async () => {
    const { ctx } = makeRun({ runId: "integ-skip-no-consumer" });
    const out = await runIntegrationStep({
      ctx,
      plan: REACT_ONLY_PLAN,
      branchResult: {
        ...greenBranch,
        gate_contract_published: true,
        contract_producers: [],
      },
      cwds: { react: "/fake/react" },
    });
    expect(out.ran).toBe(false);
    if (out.ran) throw new Error("expected ran:false");
  ...
  it("skips w/ no_contract_no_consumer when nothing was published + no consumer", async () => {
    const { ctx } = makeRun({ runId: "integ-skip-nothing" });
    const out = await runIntegrationStep({
      ctx,
      plan: REACT_ONLY_PLAN,
      branchResult: {
