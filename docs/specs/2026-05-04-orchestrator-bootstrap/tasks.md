...
- [x] 36. Add `inngest` pkg + `src/inngest/{client,serve}.ts`. _Vault RepoKit starters landed 2026-05-03; orchestrator copy at Phase 5 entry._ **Landed 2026-05-05: `inngest@^3.27.0` (resolved 3.54.2) + `hono@^4.6.0` (4.12.16) + `@hono/node-server@^1.13.0` (1.19.14). `src/inngest/client.ts` mirrors vault starter — 5 distinct events (Plan §I3) w/ Zod-validated payloads + repo allowlist (`spring-api`/`react-ui`/`agent-orchestrator`); `EventSchemas.fromZod()` wires types; `isDev` from `INNGEST_DEV=1`. `src/inngest/serve.ts` Hono handler at `/api/inngest` on `:3030` w/ `/health` check; `functions = []` at I2 (orch-run lands I3 — gated). Both files excluded from coverage per `vitest.config.ts` (`src/inngest/**`).**
- [x] 37. `.env.example` + `BootConfig` Inngest vars + README run-dev section. _Vault RepoKit starters landed 2026-05-03._ **Landed 2026-05-05: `BootConfig` extended w/ optional `INNGEST_EVENT_KEY` / `INNGEST_SIGNING_KEY` / `INNGEST_BASE_URL` (Zod `z.url()` malformed-URL guard) + derived `inngestDev` boolean from `INNGEST_DEV=1|true`. `.env.example` adds Inngest stanza w/ Cloud-rejected note + loopback-only base URL guidance. README adds Inngest section: dev wiring (`pnpm install` → `inngest:serve` + `inngest:dev`), I2-handshake-only caveat, 37a outbound-verify gate cite to ADR 0002 Appendix A. Scripts table extended w/ `inngest:serve` / `inngest:dev` / `verify:inngest-outbound`. Tests: `tests/config/env.test.ts` Phase-I2 block (3 — Inngest vars present, absent, malformed URL); `tests/inngest/client.test.ts` (7 — app id, 5 distinct events declared, schemas accept/reject for dry-plan/run/approve.spring/cancel, repo + uuid guards). Quality green: 274 tests, 0 lint warnings, tsc clean.**
...
- [x] 38. `orch-run` Inngest function listening on `orch/dry-plan.requested` + `orch/run.requested`. _Vault RepoKit starter landed 2026-05-03; orchestrator copy at Phase 5 entry._ **Landed 2026-05-05: `src/inngest/functions/orch-run.ts` mirrors vault starter — boot ritual (`expectations` → `audit-expectations` → `tf-probe` → `audit-tf-probe`) → O5 deterministic `o5-dry-run` (skip → `audit-planner-skipped` + `skipped_no_change_needed`) → `plan` (Mastra Agent stub) → `persist-plan` → `audit-plan dry_plan` → branch on `event.name`. `concurrency: [{ key: 'event.data.runId', limit: 1 }]` replaces edge-40 lockfile (task 44 pre-loaded); `retries: 2` lifted to function level (Inngest v3 — task 40 wires TF idempotency cache for `mastra-*` replay safety). Inner handler exported as `orchRunHandler` so vitest can drive both branches w/o relying on Inngest v3 internal `.fn` shape. Stubs remain in-file for `loadExpectations` / `tfProbe` / `plannerDryRun` / `mastraPlanner` / `mastraSupervisor` / `mastraSupervisorResume` / `audit` (TODO markers point at canonical wiring tasks); E2E swap-in lands once Inngest dev server can drive end-to-end. Registered in `src/inngest/serve.ts` `functions[]` (was empty at I2). Excluded from coverage per `vitest.config.ts` (`src/inngest/**`)._
- [x] 39. Per-supervisor `step.waitForEvent('orch/approve.<sup>')` between pre-approval + resume. _Vault RepoKit starter landed 2026-05-03._ **Landed 2026-05-05 in same `orch-run.ts`: execute lane fans out `mastra-spring-pre-approval` + `mastra-react-pre-approval` (parallel `Promise.all`), then `step.waitForEvent('approve-spring' | 'approve-react', { event: 'orch/approve.<sup>', match: 'data.runId', timeout: '7d' })` (parallel `Promise.all`), then `mastra-spring-resume` + `mastra-react-resume` + `audit-finalize`. `match: 'data.runId'` guards cross-run leakage (only the approval for THIS run unblocks); 7d timeout per Plan §I3. Tests: `tests/integration/inngest-orch-run.test.ts` (5 — dry-plan returns `dry_plan_done` w/ plan path + skips execute lane + zero `waitForEvent`; dry path audit order `expectations → tf-probe → planner-continue → plan`; execute returns `green` w/ both supervisors fanned + both `waitForEvent` calls present + match/timeout shape verified + event names; per-supervisor ordering `pre-approval < waitForEvent < resume`; module registers w/o throw). Subprocess-isolation spy from vault starter dropped (Node ESM `node:child_process` exports non-configurable, `vi.spyOn` rejects); equivalent guarantee provided by step-id absence assertions + existing `supervisorSpawnGuard()` in `src/workflows/plannerBranch.ts` (task 34 abuse test) — note in test header explains substitution._
